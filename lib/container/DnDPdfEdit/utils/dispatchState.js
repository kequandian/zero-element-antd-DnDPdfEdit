...
export default function handleState(state, {
  ...
  const typeMap = {
    ...
    initConfig() {
      ...
      return { ...state,
        name: name,
        fields: fotmatValue(apiField, v => v.split(',').map(f => f.field || f), []),
        // 可用的后端字段, 如 name, sex, address
        tableFields: fotmatValue(apiList, v => v.split(',').map(f => f.field || f), []),
        // table 可用的后端字段, 如 recoreds, items
    ...
    addChildLayout() {
      const node = findNode(payload.layoutId, config);
      node.items[payload.index] = new Item({ ...payload,
        parentId: node.id
      });
      return { ...state,
        config: config
